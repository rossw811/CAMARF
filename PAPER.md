...
Durability Is Not Currency: A Recency Diagnostic for Cross-Asset
Cointegration Screening, and the Honest Cost of Applying It

*(Retitled 2026-09-10, reframed around a three-act throughline Ross
approved after the genuinely point-in-time-safe 182-pair backtest result
became the paper's obvious missing conclusion, not a footnote. Prior
history, kept for provenance: retitled 2026-07-13 after a direct Monte
Carlo calibration check (§4.2.1) refuted this project's own earlier "test
miscalibration" reading of a companion observation: the durability-vs-
currency demonstration itself (§4.2) was always the load-bearing evidence
and needed no revision; only the paper's own title and framing did.)*

**The throughline, stated once here so it doesn't have to be reconstructed
from the section order:** (1) build a diagnostic that distinguishes a
durable cointegrating relationship from one whose significance is
borrowed from history that no longer holds, and demonstrate the
conflation concretely on this project's own former headline pairs; (2)
apply that diagnostic's own logic honestly, all the way through, to a
genuinely point-in-time-safe (not merely out-of-sample-backtested)
version of the full production strategy; (3) report what actually
happens, without softening it because the answer is inconvenient. The
answer is that it loses money, both in-sample and out-of-sample, robustly
across a parameter sweep. That result, not the diagnostic alone and not
the old backward-looking-screen backtest, is this paper's headline.
...
## Framing decision (locked in 2026-06-23, re-locked 2026-09-10)

**Original framing (2026-06-23), kept for provenance.** The strategy's
empirical results are of independent interest, but the calibration
finding is a genuine, exportable contribution beyond this project's own
pairs. Resolution: lead with the methodology finding, structure the
strategy chapter as the empirical demonstration of that methodology, not
a separate, weaker pitch. Concretely: contrast what naive full-sample EG
alone would have certified (this project's own former headline pairs,
NTRS/STT and SHW/UNP) against what survives the corrected,
rolling-stability-aware pipeline. Under this original framing, the
backtest results (a backward-looking-discovered pair set, Sharpe 5.2155
OOS) were meant to read as confirmation the methodology has teeth.

**Re-locked 2026-09-10, and why.** That original framing had a real gap:
the backtest it pointed to as "confirmation" never actually enforced the
paper's own diagnostic logic end to end. `coint_fraction_rolling`
distinguishes durable cointegration from borrowed significance, but the
strategy backtested against it was still discovered by a full-history
screen, the exact backward-looking process the diagnostic exists to
flag. The paper built a tool for telling durability from currency, then
never actually spent the tool on its own trading decision. Once the
project built a genuinely point-in-time-safe (episodic, causally-gated)
discovery process and backtested the full production strategy against
it honestly, the obvious next step became unavoidable: report what that
actually finds, not what the old backward-looking discovery found. **It
loses money** (§7.20): −0.679 IS / −0.834 OOS Sharpe on the Purity
PIT-safe pair set, capital-constrained, robust across a full parameter
sweep. This paper's real structure is now three acts, not two: the
diagnostic and its concrete demonstration (§4.1-§4.3, largely unchanged
from the original framing, since NTRS/STT and SHW/UNP's durability-vs-
currency conflation is real regardless of what happens next); the honest
application of that same logic to actual trading decisions (§7.20); and
the conclusion that follows from taking the diagnostic's own premise
seriously rather than stopping short of it. The pre-2026-09-10 backward-
looking-discovery backtest (§5-§7.16) is kept in full, not deleted, per
this project's "document what was tried and reverted" discipline, but is
no longer the paper's headline result: see the notice at the start of §5
for exactly how to read it now.
...
demonstrated directly and concretely, not asserted from theory, on real
WRDS/CRSP data (re-derived 2026-09-10, §4.2; the original demonstration
...
the original pre-WRDS demonstration**, reported honestly rather than
dropped (§4.2 for the full account and the disclosed price-adjustment-
convention sensitivity behind the difference); NTRS/STT alone, checked
against three negative-control pairs that show the pattern on neither
window, is sufficient evidence for the underlying claim. A companion
observation, that full-sample screens at long horizons (1D, 1M) reject
candidate pairs at rates far below their nominal false-positive rate, was
initially read as further evidence the test itself is statistically
over-conservative at those horizons, compounding the problem by also
suppressing detection of real, currently-live relationships. That reading
was tested directly with a Monte Carlo calibration study rather than left
asserted, and is refuted: the empirical false-positive rate under a
genuine, by-construction null is *elevated* relative to nominal
(7.75%-12.75% vs. 5%) and grows with horizon, the opposite of an
over-conservative test's signature, a signal of ordinary spurious
regression from shared market-wide drift among randomly-paired real
assets, not test miscalibration (§4.2.1). **This calibration study itself
still runs on pre-WRDS data and has not yet been re-derived on WRDS/CRSP
series**, flagged here as an open item (§4.2.1), not silently carried
forward as validated. The full-sample screen's near-total rejection rate
at long horizons in production reflects the test correctly guarding
against exactly this risk, not a defect, pending that re-derivation. The
durability-vs-currency conflation demonstrated by NTRS/STT remains the
real, load-bearing failure mode and needs no such correction.
...
**This diagnostic was first validated against a strategy that never
actually applied it to its own trading decision, a gap this paper's
2026-09-10 reframe exists to close (see the Framing decision above).**
An event-driven pairs-trading strategy implementing a **backward-looking-
discovered** pair set (a full-history screen, the same kind of screen the
diagnostic above exists to flag) achieved an OOS portfolio Sharpe of
**5.2155** (449 trades, chronological 20% holdout) across 26 confirmed
pairs, with IS Sharpe **5.8044** (2168 trades): a real result, cross-
checked twice more in later sessions, extensively detailed in §5-§7.19
below (deflated Sharpe, block-bootstrap significance, capital constraints,
distance-method comparison, and more), but **not this paper's headline
result**, because the pair set behind it was never required to survive a
genuinely point-in-time-safe discovery process, and §7.3.1's own smaller
test already shows that requirement is not free.

**The paper's actual headline result (§7.20): when the same production
strategy is backtested against a pair set discovered the way a live
deployment would actually have to discover it (genuinely point-in-time-
safe, episodic, causally-gated, not merely out-of-sample-backtested after
the fact), it loses money.** Capital-constrained, across the resulting
182-pair universe: IS Sharpe **−0.679**, OOS Sharpe **−0.834**. This is
not a fragile, single-configuration result: a full parameter sweep across
entry threshold, hedge method, and capital-sizing scheme finds every
out-of-sample cell negative (range −1.150 to −0.179) and all but one
in-sample cell negative. A comparison arm that PIT-confidence-weights the
old backward-looking pairs instead of excluding them (Tiered) posts a
positive Sharpe identical to unweighted Baseline, not evidence PIT-
weighting helps, but a disclosed capital-efficiency artifact of which
pairs happen to trade at all, since every pair in that comparison shares
one confidence tier at this snapshot. Read together with §7.3.1's smaller,
earlier PIT test and `PAPER_MAGNITUDE.md` §4's causal-discovery-timing
finding, this section is the full-scale, production-grade version of the
same conclusion: a screening process that can see its own future
certifies pairs, and produces backtest results, a causally-constrained
observer would not have had.

**Sections §5-§7.19, kept in full for the record, are pre-WRDS and are
not used as evidence in this paper's active argument, per Ross's standing
instruction (2026-09-10) that nothing pre-WRDS is considered for
writing.** The old backward-looking-discovery backtest, the distance-
method comparison, the Deflated Sharpe Ratio and block-bootstrap
significance tests, and the capital-constraint analysis in those sections
all ran on yfinance-era data predating this project's WRDS-primary
sourcing (§3, landed 2026-08-01), and are kept only as historical
provenance of what this paper tried before WRDS, not as claims the paper
currently makes. One exception survives on its own merits regardless of
data source: the calendar-padding-artifact finding (§4.5) is a purely
mechanical property of fixed-window rolling statistics on calendar-padded
series, independent of which price source produced the underlying data,
so it remains an active, citable finding. See the notice at the start of
§5 for exactly how to read the boundary between what's active and what's
historical.
...
### 4.2 Durability vs. currency: what a full-sample cointegration test actually answers [Re-derived 2026-09-10 on real WRDS/CRSP data, replacing the original pre-WRDS (yfinance-era) demonstration per Ross's standing instruction: nothing pre-WRDS is considered for writing in this paper]

**The core demonstration, re-run on real CRSP total-return-adjusted daily
closes (`research/durability_vs_currency_wrds.py`, reusing `analysis.py`'s
own production `_eg_worker` unmodified: same test, only the data source
changes).** Full-sample EG p-value vs. the identical test restricted to
...
project's own original headline pair passes full-sample EG with
overwhelming significance (p≈0.00005) while failing the identical test on
just the last five years (p=0.561). A full-sample screen over 50+ years
of CRSP history answers a different question than a live deployment
actually needs answered: "was this relationship ever cointegrated," not
"is it cointegrated now." NTRS/STT clears the former bar decisively while
having already failed the latter, because nothing in a full-sample test
can distinguish a relationship that held throughout from one that held
for decades and then broke.

**SHW/UNP does NOT replicate as cleanly on WRDS data, reported honestly
rather than dropped or forced to fit.** The original pre-WRDS (yfinance-
adjusted-close) version of this demonstration reported SHW/UNP at
full-sample p=0.004 (clearly significant) vs. last-5-years p=0.265
(clearly not), the same clean pattern as NTRS/STT. On CRSP total-return-
adjusted data, SHW/UNP's full-sample p-value is 0.061 (not significant at
the conventional 0.05 threshold) and its last-5-years p-value is 0.054
(borderline, not a clear failure either). This is a real, disclosed
sensitivity of this specific pair's demonstration to the price-adjustment
convention used (CRSP total-return-adjusted vs. yfinance's own adjusted-
close, which handle dividends and corporate actions differently over a
50+-year window), not a bug in the re-derivation or a reason to doubt the
underlying point: NTRS/STT alone is sufficient to demonstrate that a pair
...
to pairs that already pass the primary full-sample screen: a
rolling-window recency check downstream of a test that cannot itself
distinguish "held once" from "holds now" is exactly the tool needed to
close this specific gap, which is why §4.3 builds it directly on this
finding.

#### 4.2.1 A companion observation, tested directly rather than left asserted, and refuted [PRE-WRDS, not yet re-derived: flagged 2026-09-10, not silently carried forward]

**Disclosure added 2026-09-10**: everything in this subsection, the raw
rejection-rate table, the Monte Carlo null-construction, and the
calibration result, runs on this project's pre-WRDS (yfinance-era)
candidate universe and price cache. Per Ross's standing instruction that
nothing pre-WRDS is considered for writing, this subsection's own
conclusion should be read as **historically informative, not as current
evidence** until it is re-derived against the WRDS/CRSP universe (§3):
a real, scoped, not-yet-started task (the 400-null-pairs-per-timeframe
Monte Carlo harness needs re-pointing at WRDS-cached series and a fresh
synthetic ground-truth check before trusting a re-run, not a quick swap).
The refutation itself (EG's near-total 1D/1M rejection rate is correct
test behavior, not over-conservatism) may well still hold on WRDS data;
it has simply not been checked yet, and this paper does not claim it has.
...
**Re-scoped 2026-09-10, per the Framing decision above.** §5-§7.16 are kept in full as Act One:
the durability-vs-currency diagnostic and the historical backward-looking-discovery backtest it
was originally validated against. Read them as that, not as the paper's current headline result.
**§7.20 is the paper's actual current empirical reality**: the same production strategy,
backtested honestly against a genuinely point-in-time-safe discovery process. It reports a
real, robust, negative result. If you are reading this paper for its bottom line, start there,
not here.
...
**STALE AS OF 2026-09-13, RE-DERIVATION NEEDED — flagged per this project's own "never cite from
memory, re-derive" rule, not silently updated.** Every result in §6.1-6.3 below was derived against
a 29-pair WRDS-primary production manifest that has since been superseded: that manifest is now
**17 pairs**, after a same-night session found and removed genuine contamination the 29-pair count
included — 22 same-GVKEY-number duplicate pairs (already excluded before reaching 29), plus,
discovered and fixed 2026-09-13, 2 more WRDS ticker↔PERMNO alias-duplicate pairs and 12 SPAC
NAV-clustering spurious-cointegration pairs. **§6.2's own disclosed gap is no longer a gap**: the
`stats.py` cache-path bug it describes ("`'1d'` → `'1day'`" not covering WRDS's `'1D'` label) was
found and fixed 2026-09-12/13 (root-caused: the dict was independently drifted in two places, now
aliased to the single already-correct `DataStore._TF_SAFE` source) — re-running §6.2 would now
produce a real Huber/MM estimate for the WRDS-daily pairs instead of the disclosed NaN gap. Two
more real bugs affecting these results were also found and fixed the same session (`SpreadModel.
half_life_ar1`'s off-by-one, `SpreadModel.fit_pair`'s `clean_mask` `GapFlag`-semantics violation) —
both directly affect §6.3's backtest-trade-generation numbers (12/29 pairs trading). None of §6.1-
6.3 has been re-run against the corrected 17-pair manifest with all fixes applied; the numbers
below are the real, honestly-reported 2026-09-10 result on the pair set and code state that existed
then, not wrong, just superseded. Full account: `Development.md`'s 2026-09-12/13 entries,
`docs/HANDOFF.md`.

### 6.1 Confirmatory cointegration tiers (EG + KPSS + PO) [Re-derived 2026-09-10 on the current WRDS-primary production pair set, replacing the pre-WRDS 26-pair result per Ross's standing instruction]
...
**Real result, run 2026-09-10 via `stats.py`'s own production
`run_confirmatory_cointegration()`, unmodified, against the current
29-pair WRDS-primary production manifest** (`output/results/{1day,3min,
4hr}/pairs.parquet`, the full-universe EG+BH-FDR cascade, not the old
pre-WRDS 26-pair 1h-heavy set):
- Gold (n_confirm = 3): **0 pairs**
- Silver (n_confirm = 2): **2 pairs**
- Bronze (n_confirm = 1, EG-only): **27 pairs**
- Conflicts (EG confirms, KPSS rejects): **2 pairs**

**This is a real, disclosed reversal of the old result's tier distribution
(17 gold/8 silver/0 bronze), not a data-quality regression, and not
directly comparable to it.** The two pair sets are different populations
almost entirely: the old set was concentrated at 1h with a DD-hub cluster
from a tightly-curated screen; the current set is 27 pairs @1D, 1 @3m, 1
@4h from a much broader, less-curated full-universe cascade (§3). Daily
bars carry proportionally more noise per bar relative to the spread's own
mean-reversion speed than hourly bars do for a KPSS/PO stationarity test,
which plausibly explains a real part of the shift toward bronze, but this
has not been separately tested and is stated as a plausible mechanism,
not a confirmed one. The honest reading: at real WRDS-primary scale, the
current production pair set mostly clears only the primary EG screen, not
the two independent confirmatory tests this section was designed to
require, a genuinely different and more cautious picture than the old
pre-WRDS snapshot painted. Full account: `output/research/confirmatory_
cointegration_tiers_wrds_29pair.parquet`.
...
**Attempted re-derivation 2026-09-10 against the current 29-pair
WRDS-primary set found a real, disclosed infrastructure gap, not a data
issue.** `stats.py`'s `run_robust_hedge_ratios()` reconstructs raw prices
for the Huber/MM fit via a cache-path lookup keyed to the old IBKR/
yfinance timeframe naming convention (`"1d"` → `"1day"`), which does not
cover WRDS's own `"1D"` (capital D) label; for every WRDS-sourced daily
pair, this silently returns NaN rather than erroring, so the function
appeared to run cleanly while producing no robust estimate for 27 of the
29 current pairs. This is a pre-existing gap in `stats.py`'s cache-path
logic that this replication effort surfaced, not something introduced
here, and not fixed as part of this pass (a cache-path fix is a real code
change needing its own care, out of scope for a claims-replication
sweep). **What did compute cleanly**: the 2 remaining pairs from the
legacy standard-screen set (KVUE/KMB@3m, PNC/ZION@4h, both non-daily,
whose cache paths the existing lookup does cover) show low divergence
between OLS and Huber/MM (max 1.70%), well under the old 5% threshold,
but this is a 2-pair sample and not evidence either way about the
27 WRDS-daily pairs the original 9/26 finding was actually about. The
original pre-WRDS 9/26-pairs-diverge-by-5%+ finding is not replicated,
positively or negatively, until the cache-path gap is fixed and this is
re-run. Flagged as a real open item, not silently dropped.

### 6.3 Extreme value theory (EVT / GPD tail risk) [Re-derived 2026-09-10 on real trades from the current 29-pair WRDS-primary production set]

Generalized Pareto Distribution (GPD) fit to spread losses above the 95th
percentile per pair.

**Real re-derivation, and a real data-thinness finding that limits what
it can show.** A full IS backtest was run against the current 29-pair
production manifest (`backtest.py --pairs-override`, building the
override file from the actual `output/results/{1day,3min,4hr}/
pairs.parquet` rows, not a bare symbol list, since the engine reads
hedge-ratio and coint-fraction fields directly off each pair row). Real
result: **only 12 of the 29 pairs generate any trade at all** (290 trades
total, IS Sharpe 0.2389), a much thinner, more fragmented trading pattern
than the old tightly-curated 1h set. Of those 12, `stats.py`'s
`run_evt_tail_risk()` (unmodified) finds only **2 pairs with enough
tail exceedances for a valid GPD fit at all: KVUE/KMB and PNC/ZION, both
fat-tailed** (the same two legacy standard-screen pairs carried over from
before the full-universe cascade). The remaining 27 pairs, all from the
new WRDS-primary daily cascade, have too few trades (many zero) for a
GPD fit to be meaningful. **This is not a replication of the old
17/26=65% finding, positive or negative: the current production set is
too thin to test the same claim.** The honest, disclosed reading:
whatever tail-risk properties the new full-universe-cascade pairs have
cannot be assessed from this backtest, a real limitation of the current
production set's trade density, not evidence the old finding was wrong.
...
|---|---|---|---|---|
| Baseline | 296 | $73,596 | 5.2443 | n/a |
...
| Risk-parity | n/a | **5.8689** | n/a | 296 | $62,490 |
| Neg-hedge | n/a | 5.4460 | n/a | 304 | $77,740 |
...
### 7.20 Act Three: Applying the Diagnostic Honestly, End to End: the Production Strategy Backtested Against a Genuinely Point-in-Time-Safe Discovery Process [DRAFTED: 2026-09-10, real backtest completed 2026-08-14]

§7.17 disclosed, but did not resolve, a real PIT-safety gap: every comparison arm in this paper
through §7.19, and the headline backtest itself (§5-§7.16), sources its pairs from a full-history
screen, the exact backward-looking discovery process §4's own diagnostic exists to flag. This
section closes that loop. It does not introduce a new result computed for this rewrite: the
backtest below was run 2026-08-14 (Session 31, `Development.md`), extensively cross-validated at
the time, and has sat in this project's canonical memory and `README.md`'s own "Current Results"
section for weeks without being promoted into this paper's own narrative: promoting it here, as
the paper's actual headline rather than a disclosed footnote, is what changed 2026-09-10.

**The discovery process, made genuinely point-in-time-safe.** A pair qualifies not by a single
whole-history verdict but via a rolling, episodic re-confirmation (the same machinery
`PAPER_MAGNITUDE.md` §3 documents in full), gated so that no window's confirmation can see data
from after its own as-of date. Applied against the corrected, WRDS-primary universe (§3), this
produces a **182-pair set** (170 WRDS/1D, 6 intraday/1h, 6 intraday/4h;
`output/research/episodic_confirmed_pairs_adapter_output.parquet`, BUG-D112-fixed): the **Purity**
arm, no full-history-screen pairs mixed in. A second **Hybrid** arm unions Purity with the 3
non-PIT-safe standard-screen pairs (§3), tagged by source, for comparison. A third and fourth,
**Tiered** and **Baseline**, apply the standard full-history-discovered pairs with and without a
PIT-confidence tier weight respectively, kept as a comparison, not as evidence favoring the old
discovery process, for the reason given below.

**Result, capital-constrained ($100k fixed sizing, `backtest.py --capital-sim`), both in-sample
and out-of-sample:**

| Arm | IS Sharpe | OOS Sharpe | Discovery process |
|---|---|---|---|
| **Purity** | **−0.679** | **−0.834** | Genuinely PIT-safe (182 pairs) |
| Hybrid | −0.442 | −1.125 | PIT-safe + 3 standard-screen pairs mixed in |
| Tiered | +1.417 | +0.630 | Standard full-history screen, PIT-confidence-weighted |
| Baseline | +1.417 | +0.630 | Standard full-history screen, unweighted |

**Read the Tiered/Baseline result for what it actually is, not what it looks like at a glance.**
Tiered and Baseline are identical here, not because PIT-confidence weighting was tested and found
useless, but because it was never actually tested: all 3 standard-screen pairs share exactly one
confidence tier at this snapshot (`full_history_only`), so `compute_pit_confidence_weights()` has
nothing to differentiate between them and degenerates to unweighted Baseline. Their positive Sharpe
is a capital-efficiency artifact of which specific pairs happen to trade at all under this position
sizing, not evidence that full-history discovery, PIT-weighted or not, produces a working strategy.
The only two arms that actually answer this paper's question, both PIT-safe pair discovery to
varying degrees, are Purity and Hybrid. **Both are negative, both in-sample and out-of-sample.**

**Robustness, not a single lucky (or unlucky) configuration.** A full parameter sweep
(`research/parameter_sensitivity_screen.py`, IS + OOS, `--capital-sim`, against the Purity
universe) confirms this is not an artifact of one entry-threshold or hedge-method choice:

| Parameter | Split | Sharpe range | Min | Max |
|---|---|---|---|---|
| `ENTRY_ZSCORE` | OOS | 0.971 | −1.150 | −0.179 |
| hedge method | OOS | 0.591 | −1.102 | −0.511 |
| capital-sizing method | OOS | 0.007 | −0.834 | −0.827 |
| `ENTRY_ZSCORE` | IS | 1.182 | −1.036 | +0.146 |
| hedge method | IS | 0.078 | −0.746 | −0.668 |
| capital-sizing method | IS | 0.203 | −0.679 | −0.476 |

Every OOS cell across all three sweeps is negative. The IS entry-zscore sweep touches positive
territory at its single most permissive setting (+0.146), the only cell in the entire grid that
does; every other cell, IS and OOS alike, is negative. This is not a strategy that is marginal
under realistic constraints and wins under a friendlier parameterization. It loses under
essentially every parameterization tested.

**What this does and does not show.** It does not show that CAMARF's underlying cointegration
detection is wrong, or that the pairs the full-history screen finds are spurious: §7.3.1 and
`PAPER_MAGNITUDE.md` §4 already make the narrower, better-evidenced version of that causal-validity
argument on smaller, more tightly-controlled tests. What this section adds is scale and honesty:
the full production strategy, every hedge method, every sizing scheme this project has built,
backtested against pairs discovered the way a live deployment would actually have to discover
them, loses money. The diagnostic this paper opens with (§4) exists specifically to tell durable
cointegration from borrowed significance. Applied to a single pair, it changes which pairs you'd
trade. Applied honestly to the entire discovery process behind a real strategy, it changes whether
you'd trade at all.

Files: `output/research/episodic_confirmed_pairs_adapter_output.parquet`, `research/build_
comparison_arm_pairs.py` (`purity_pairs.parquet`, `hybrid_pairs.parquet`, `tiered_pairs.parquet`),
`output/backtest/portfolio_layer1{,_holdout}_pairsoverride.parquet`,
...
the entry criteria. Confirmed directly: `backtest.py`'s entry gate is purely `|z_rolling| >=
ENTRY_ZSCORE`; no volatility-squeeze or price-momentum confirmation existed anywhere. A real TTM-
style squeeze indicator and RSI momentum were already computed by `analysis.py`'s `VolumeStructure`
step but never reached the entry logic or `ml.py`'s feature set.

Built as 3 independently-tested STORM comparison arms (`--storm-squeeze-gate`,
`--storm-momentum-gate`, `--storm-squeeze-momentum-gate`) against the rebuilt 1,375-pair Purity
pool. All 3 flip the unconstrained Sharpe from the pool's own -0.218 to +0.35/+0.39/+0.43 (IS) and
+0.45/+0.24/+0.50 (OOS) — real, out-of-sample-confirmed. Validated with a random-subsample control
(2,000 size-matched random draws per gate from the ungated pool): **all 3 gates' real Sharpe
landed at the 100th percentile, p≈0.0000** — the strongest statistical confirmation this project
has produced that a result is a genuine selection effect, not sampling variance. The
capital-constrained headline metric does not yet track this improvement cleanly across any gate —
see §7.16's own earlier finding on chronological trade-admission bias for likely-related prior
context; not yet resolved, next investigation step per Ross's own explicit ordering (validate
signal → capital-allocation sweep → parameter sensitivity screen).

Full account: `docs/FINDINGS.md` #68, `docs/HANDOFF.md` 2026-09-15/16/19/20 entries (run-by-run
numbers and dates).

### 7.22 A Capital-Constraint Luck Check, a Hierarchical DSR Correction, and a Sweep-Design Bug
Found While Investigating Both [DRAFTED: 2026-09-21]

Three threads, closing out the ordering Ross set for this leg of the work (validate signal →
capital-allocation sweep → parameter sensitivity screen), each producing a real result rather than
a clean confirmation. **(1)** Built a direct test of whether `--capital-sim`'s chronological trade
admission preferentially keeps better trades: it does not. At $100k IS on the momentum-gate arm,
taken trades show Sharpe -0.27 vs. skipped trades' +0.40, and the taken subset sits at just the
